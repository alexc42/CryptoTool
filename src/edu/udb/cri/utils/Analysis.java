...
public class Analysis {
	...
	public int getMcd(ArrayList<String> arrLista, int sizeSilaba) {
        ...
        //Primero obtengo el menor numero del array para probar a dividir
        ...
        //Obteniendo la variable "menor" el menor n�mero del array      
        while (probar_siguiente==true){     
            //Ahora comprobar si "menor" es divisible por todos los n�meros del array
            for ( A=0 ; A < ArrayMcd.size() && no_es_mcd==false; A++){
                if (ArrayMcd.get(A) % menor != 0) {
                    no_es_mcd=true;
                }
            }
            //Si no lo es, tendr� que probar con el siguiente n�mero, que ser� menor-1
            if (no_es_mcd){
                menor --;
                no_es_mcd=false;
            } else{
                //En caso de que todos los n�meros sean divisible por menor, salir del bucle principal while se encontro mcd
